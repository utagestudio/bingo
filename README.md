...
ブラウザで表示されたURLを開き、項目を入力します。通常状態では盤面セルをクリックして開閉できます。`挨拶をもらう x10` のように行末へ `x回数` を付けると、クリックでカウントが増え、指定回数に達したらセルが開きます。セル位置を手動で入れ替えたい場合だけ、項目入力エリア付近の並び替えモードをONにしてドラッグします。
